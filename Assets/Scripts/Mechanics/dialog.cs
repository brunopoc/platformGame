...
public class Dialog : MonoBehaviour
{
    ...
    void Start()
    { //Comçe setando algumas váriaveis
    ...
    void Update()
    { //Verifica constantemente se existe texto a exibir e se o botão z está sendo pressionado
        ...
        {
            StartCoroutine(AnimateText(MyMsg));
        }
        if (animaAction == "end")
        {
            animationDialog("end");
    ...
    { //Função que exibe o texto
        ...
        while (i < strComplete.Length && hasFinishCheck == false)
        { //Enquanto houver informação na váriavel, ela é passada uma a uma para a 'str'
            str += strComplete[i++];
            yield return new WaitForSeconds(0.15f);
            if (pressButtonZ == true)
            { //Se o botão Z é pressionado o texto pula para o final
            ...
            { //Quando chegar na ultima ela avisa que terminou
                hasFinishCheck = true;
                showDiolog = false;
                i = 0;
            }
